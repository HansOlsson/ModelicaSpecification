...
\item
  If the record has \lstinline!annotation(CStructName="CName")! the struct has the name \lstinline[language=C]!CName!, \cref{annotations-for-records-in-external-functions}.
\item
...
\hline
\hline
{\lstinline!Include!} & Include directives & \Cref{modelica:recordInclude}\\
{\lstinline!IncludeDirectory!} & Directories containing header files & \Cref{modelica:recordIncludeDirectory}\\
{\lstinline!CStructName!} & Name of struct in C & \Cref{modelica:recordCStructName}\\
\hline
\end{tabular}
\end{center}

The first two can specify either a scalar value or an array of values and work identically to the corresponding ones for external functions.

\begin{annotationdefinition}[recordInclude]
\begin{synopsis}\begin{lstlisting}
/*literal*/ constant String Include
\end{lstlisting}\end{synopsis}
\begin{semantics}
The {\lstinline!annotation(Include="insertedCode")!}, used to insert a struct definition.
When using the struct, the \lstinline!"insertedCode"! shall be present at the top level somewhere before the use (similar to where include directives are typically placed).
It should declare the struct with members in C, but they may be aligned differently, they may be in a different order, and additional padding or fields may exist.
...
\begin{annotationdefinition}[recordIncludeDirectory]
\begin{synopsis}\begin{lstlisting}
/*literal*/ constant String IncludeDirectory
\end{lstlisting}\end{synopsis}
\begin{semantics}
...
\begin{annotationdefinition}[recordCStructName]
\begin{synopsis}\begin{lstlisting}
/*literal*/ constant String CStructName
\end{lstlisting}\end{synopsis}
\begin{semantics}
The {\lstinline!annotation(CStructName="CName")!} gives the name of the struct in C.
