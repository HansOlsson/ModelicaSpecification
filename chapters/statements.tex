...
The syntax of a statement is given by \productionref{statement} in the grammar.
...
The expression must not have higher variability than the assigned component, see \cref{variability-of-expressions}.
...
Here, each \productionref{expression} in the grammar (representing any condition of an \lstinline!if!-clause) must be a scalar \lstinline!Boolean! expression.
...
One or zero of these branches is selected, by evaluating their conditions sequentially until a condition that evaluates to true is found.
...
In an algorithm section, the body of the selected branch is then executed.
...
The difference compared to the when-statements is that, e.g., \lstinline!pre! may only be used on continuous-time real variables inside a \lstinline!when!-clause body and not inside these if-statements.
